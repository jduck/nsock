...
void
inet_co_init_bad_dport(void)
{
   nsock_t *nst;
   u_int ns_errno;
   
   /* print what we are testing.. */
   printf("%-32s", "inet_co_init_bad_dport");
   fflush(stdout);
   
   /* try to initialize the struct */
   ns_errno = NSERR_SUCCESS;
   nst = nsock_connect_init(PF_INET, SOCK_STREAM, NULL, TCP_HOST_INVALID_PORT, NSF_NONE, &ns_errno);
   if (ns_errno == NSERR_INET_CO_DST_BAD_PORT)
     {
