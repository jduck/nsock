 ...
 * inet connect init resolver failure
 * 
...
void
inet_co_init_bad_src(void)
{
   nsock_t *nst;
   u_int ns_errno;
   
   /* print what we are testing.. */
   printf("%-32s", "inet_co_init_bad_src");
   fflush(stdout);
   
   /* try to initialize the struct */
   ns_errno = NSERR_SUCCESS;
   nst = nsock_connect_init(PF_INET, SOCK_STREAM, TCP_HOST_INVALID, TCP_HOST_VALID, NSF_NONE, &ns_errno);
   if (ns_errno == NSERR_INET_CO_SRC_BAD_HOST)
     {
