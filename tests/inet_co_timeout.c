...
void
inet_co_timeout(void)
{
   nsock_t *nst;
   u_int ns_errno;
   
   /* print what we are testing.. */
   printf("%-32s", "inet_co_timeout");
   fflush(stdout);
   
   /* try to initialize the struct */
   if (!(nst = nsock_connect_init(PF_INET, SOCK_STREAM, NULL, TCP_HOST_NO_ROUTE, NSF_NONE, &ns_errno)))
     {
