...
void
inet_listen_bind_failure(void)
{
   nsock_t *nst;
   u_int ns_errno;
   
   /* print what we are testing.. */
   printf("%-32s", "inet_listen_bind_failure");
   fflush(stdout);
   
   /* try to initialize the struct */
   nst = nsock_listen_init(PF_INET, SOCK_STREAM, TCP_HOST_BIND_FAILS, 1, NSF_NONE, &ns_errno);
   if (ns_errno == NSERR_BIND_FAILED)
     {
