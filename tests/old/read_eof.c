 ...
 * valid tcp connection and read end of file
 * 
...
read_eof(void)
{
   ...
   if (!(nst = nsock_tcp_init_connect(NULL, TCP_HOST_WRAPPED,
   ...
   nr = nsock_read(sd, buf, sizeof(buf), 0);
   close(sd);
   if (nr == -1)
     printf("FAILED   nsock_read failed: %s\n", strerror(errno));
   else if (nr == 0)
     {
	printf("PASSED   nsock_read returned EOF\n");
	return;
     }
   else
     printf("FAILED   nsock_read actually got something :(\n");
   _exit(1);
}
