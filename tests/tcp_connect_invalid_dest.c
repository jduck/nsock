...
tcp_connect_invalid_dest(void)
{
   ...
   printf("%-32s", "tcp_connect_invalid_dest");
   fflush(stdout);
   
   /* try to initialize the struct */
   if (!(nst = nsock_tcp_init_connect(NULL, TCP_HOST_INVALID,
   ...
   if (sd < 1)
     {
	if (sd == -4) 	/* XXX: hardcoded return value */
	  {
