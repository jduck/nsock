...
void
tcp_connect_invalid_src_port(void)
{
   char eb[256];
   int sd;
   nsocktcp_t *nst;
   ...
   printf("%-32s", "tcp_connect_invalid_src_port");
   fflush(stdout);
   
   /* try to initialize the struct */
   if (!(nst = nsock_tcp_init_connect(TCP_HOST_INVALID_PORT, TCP_HOST_VALID,
   ...
   if (sd < 1)
     {
	if (sd == -2) 	/* XXX: hardcoded return value */
	  {
