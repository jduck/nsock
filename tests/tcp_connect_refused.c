...
tcp_connect_refused(void)
{
   ...
   printf("%-32s", "tcp_connect_refused");
   fflush(stdout);
   
   /* try to initialize the struct */
   if (!(nst = nsock_tcp_init_connect(NULL, TCP_HOST_LOCAL_UNUSED,
   ...
   if (sd < 1)
     {
	if (errno == ECONNREFUSED) 	/* XXX: hardcoded return value */
	  {
