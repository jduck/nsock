...
tcp_connect_timedout(void)
{
   ...
   printf("%-32s", "tcp_connect_timedout");
   fflush(stdout);
   ...
   /* try to connect */
   sd = nsock_tcp_connect(nst, 1);
   nsock_tcp_free(&nst);
   if (sd < 1)
     {
	if (errno == ETIMEDOUT) 	/* XXX: hardcoded return value */
	  {
