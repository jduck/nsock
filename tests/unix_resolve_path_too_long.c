...
void
unix_resolve_path_too_long(void)
{
   nsock_t *ns;
   
   
   /* print what we are testing.. */
   printf("%-32s", "unix_resolve_too_long");
   fflush(stdout);
   ...
   if (nsock_unix_resolve(UNIX_SOCK_PATH_TOO_LONG, ns) != NSERR_SUCCESS)
     {
	if (ns->ns_errno == NSERR_UNIX_PATH_TOO_LONG)
	  printf("PASSED   %s\n", nsock_strerror_full(ns));
	else
	  printf("FAILED   %s\n", nsock_strerror_full(ns));
     }
   else if (strcmp(UNIX_PATH_TOO_LONG, ns->unix_path) != 0)
